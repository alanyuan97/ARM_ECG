%THE IDEA OF THIS FILE WAS TO DETECT AS A PEAK ONLY THE MAXIMUM OF EVERY
%PERIOD ABOVE THE THRESHOLD INSTAD OF EVERY TIME THE DIRECTION OF THE CURVE
...
%         if (filtered(i-2)<filtered(i-1)) && (filtered(i)<filtered(i-1))
%             if (filtered(i-1)>THR1(i-1)) && (count>refractory_period)
%                 SPK(i) = 0.125*filtered(i-1) + 0.875*SPK(i-1);
%                 RR = [RR; count];
